...
import edu.metrostate.ics342.mediatracker.data.model.toRequest
...
class DefaultMediaRepository(sessionRepository: SessionRepository) {
    ...
    suspend fun getPriorities(): List<Priority> {
        ...
        if (!response.isSuccessful) {
            val message = parseErrorMessage(response) ?: "Failed to load priorities (${response.code()})"
            error(message)
        }
    ...
    suspend fun updatePriorities(priorities: List<Priority>): List<Priority> {
        priorities.forEach { priority ->
            val response = api.updatePriority(priority.toRequest())
            if (!response.isSuccessful) {
                val message = parseErrorMessage(response) ?: "Failed to update priority ${priority.mediaId} (${response.code()})"
                error(message)
            }
        }
        return priorities
